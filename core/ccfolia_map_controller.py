...
class CCFoliaMapController:
    ...
    def get_board_state(self):
        ...
        for p in raw:
            ...
            result.append({
                ...
                'vx': p['vx'], 'vy': p['vy'],   # ビューポート座標（デバッグ用）
            })
        return result
    ...
    def move_piece_by_current_pos(self, cur_grid_x, cur_grid_y, dst_grid_x, dst_grid_y):
        pieces = self.get_board_state()
        targets = [p for p in pieces
                   if p['grid_x'] == cur_grid_x and p['grid_y'] == cur_grid_y]
        if not targets:
    ...
    def pan_map(self, direction, grid_amount=1):
        key = {'up': Keys.ARROW_UP, 'down': Keys.ARROW_DOWN,
               'left': Keys.ARROW_LEFT, 'right': Keys.ARROW_RIGHT}.get(direction)
        if not key:
            return
        try:
            self.driver.find_element(By.CSS_SELECTOR,
                '[class*="map"],[class*="board"],[class*="field"]').click()
        except Exception:
    ...
    def _drag(self, pieces, target, tgt_px_x, tgt_px_y):
        ...
        print(f"   🖱️  scale={scale:.4f}  dpi={dpi:.2f}"
...
MAP_TOOLS = [
    {"type": "function", "function": {
        "name": "get_board_state", "description": "全駒の位置情報を取得",
        "parameters": {"type": "object", "properties": {}}}},
    {"type": "function", "function": {
        "name": "move_piece", "description": "img_hashで駒を特定してグリッド座標に移動",
        "parameters": {"type": "object",
            "properties": {"img_hash": {"type": "string"},
                           "grid_x":   {"type": "integer"},
                           "grid_y":   {"type": "integer"}},
    ...
    {"type": "function", "function": {
        ...
        "parameters": {"type": "object",
            "properties": {"cur_grid_x": {"type": "integer"},
    ...
    {"type": "function", "function": {
        "name": "pan_map", "description": "マップをスクロール",
        "parameters": {"type": "object",
...
def execute_map_tool(ctrl, tool_name, args):
    if tool_name == "get_board_state":           return ctrl.get_board_state()
    if tool_name == "move_piece":                return ctrl.move_piece(**args)
    if tool_name == "move_piece_by_current_pos": return ctrl.move_piece_by_current_pos(**args)
    if tool_name == "pan_map":                   return ctrl.pan_map(**args)
